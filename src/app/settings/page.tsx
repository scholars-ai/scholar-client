  ...
  return <main className="page">
    ...
    {loading ? <div className="empty panel">正在读取调度配置…</div> : <form onSubmit={save}>
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card panel"><h2>Memory Reflector</h2><div className="stack"><label className="checkbox"><input type="checkbox" checked={settings.memoryReflect.enabled} onChange={(e) => update("memoryReflect", { ...settings.memoryReflect, enabled: e.target.checked })} />启用每周反思</label><div className="field"><label htmlFor="reflect-weekday">星期（1=周一，7=周日）</label><input className="input" id="reflect-weekday" type="number" min={1} max={7} value={settings.memoryReflect.weekday} onChange={(e) => update("memoryReflect", { ...settings.memoryReflect, weekday: Number(e.target.value) })} /></div><div className="field"><label htmlFor="reflect-time">执行时刻</label><input className="input" id="reflect-time" type="time" value={settings.memoryReflect.time} onChange={(e) => update("memoryReflect", { ...settings.memoryReflect, time: e.target.value })} /></div><div className="field"><label htmlFor="reflect-timezone">时区</label><input className="input" id="reflect-timezone" value={settings.memoryReflect.timezone} onChange={(e) => update("memoryReflect", { ...settings.memoryReflect, timezone: e.target.value })} /></div><div className="field"><label htmlFor="reflect-lookback">回看天数</label><input className="input" id="reflect-lookback" type="number" min={1} max={90} value={settings.memoryReflect.lookbackDays} onChange={(e) => update("memoryReflect", { ...settings.memoryReflect, lookbackDays: Number(e.target.value) })} /></div></div></section>
      </div>
