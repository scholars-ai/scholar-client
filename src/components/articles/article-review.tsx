...
export default function ArticleReview({ articleId }: { articleId: string }) {
  ...
  return (
    <main className="page article-page article-detail-page">
      <div className="page-heading">
        ...
        <Button asChild variant="outline"><Link href="/articles"><ArrowLeft aria-hidden="true" />返回文章列表</Link></Button>
      </div>

      {error && <div className="error panel">{error}</div>}
      {notice && <div className="success panel">{notice}</div>}
      {loading && <div className="empty panel">正在加载原稿、版本链与评分…</div>}
      {detail && !loading && (
        <section className="article-review">
              ...
              <div className="version-tabs" aria-label="文章版本">
                {detail.versions.map((version) => (
                  <Button asChild variant={version.id === detail.article.id ? "primary" : "default"} size="sm" key={version.id}>
                    <Link href={`/articles/${version.id}`}>v{version.version} · {statusLabels[version.status]}</Link>
                  </Button>
                ))}
              </div>
